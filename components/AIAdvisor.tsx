  ...
  const handleSend = async () => {
    ...
    try {
      ...
      const response = await ai.models.generateContent({
        ...
        contents: history,
        config: {
          systemInstruction: "You are the Little Elm High School TSA Pro Advisor. Your mission is to help students choose competition events for the 2025-2026 year. Be encouraging, professional, and knowledgeable about the 40+ events. If they mention interests, map them to specific events like Animatronics, Webmaster, Coding, or Structural Design. Keep responses concise and formatted for a chat interface. IMPORTANT: Do NOT use markdown asterisks (*), bolding (**), headers (#), or bullet points (*). Write in plain text paragraphs or use simple dashes (-) for lists if absolutely necessary. Use emojis sparingly to look friendly. You must ONLY answer questions about TSA events and guidelines. Do NOT generate content (e.g., do not write essays, code, or create designs). If asked to generate content, politely explain that you can guide them on how to do it but cannot do it for them.",
  ...
  return (
    <div className="relative group p-[3px] rounded-2xl bg-gradient-to-r from-electric-500 via-gold-500 to-gold-400 bg-[length:400%_400%] animate-gradient-xy shadow-xl overflow-hidden">
      ...
      <div className="absolute inset-0 pointer-events-none z-0">
          ...
          <div className="absolute top-1/2 right-1/4 w-1.5 h-1.5 bg-gold-300 rounded-full animate-pulse opacity-50"></div>
      </div>

      <div className="bg-space-800 rounded-xl flex flex-col h-[500px] overflow-hidden relative z-10">

        {/* Header */}
        <div className="p-4 border-b border-space-500/30 bg-space-700/50 flex items-center justify-between">
          <div className="flex items-center gap-3">
            <div className="w-10 h-10 bg-gradient-to-br from-electric-500 to-electric-700 rounded-xl flex items-center justify-center text-white shadow-lg shadow-electric-500/20 relative overflow-hidden group-hover:scale-105 transition-transform duration-500">
              <div className="absolute inset-0 bg-white/20 animate-pulse"></div>
            </div>
            <div>
              <h3 className="font-bold text-ink flex items-center gap-2">
                TSA Pro Advisor
                <span className="text-[10px] bg-gradient-to-r from-electric-500 to-electric-600 text-white px-2 py-0.5 rounded-full font-bold shadow-sm">
                  AI
                </span>
              </h3>
              <p className="text-xs text-ink-muted">Powered by Gemini Pro</p>
            </div>
          </div>
          <button
...
Reset
        ...
        {/* Messages */}
        <div ref={scrollRef} className="flex-1 overflow-y-auto p-6 space-y-4 bg-space-900/50">
          {messages.map((msg, i) => (
            <div key={i} className={`flex ${msg.role === 'user' ? 'justify-end' : 'justify-start'} animate-slide-up`}>
              <div className={`max-w-[85%] flex gap-3 ${msg.role === 'user' ? 'flex-row-reverse' : 'flex-row'}`}>
                <div className={`w-8 h-8 rounded-lg shrink-0 flex items-center justify-center border transition-all duration-300 ${
                  msg.role === 'bot'
                    ? 'bg-electric-500 border-electric-500 text-white shadow-md scale-100'
                    : 'bg-space-700 border-space-500/30 text-ink-muted'
                }`}>
                  {msg.role === 'bot' ? 'AI' : 'You'}
                </div>
                <div className={`p-4 rounded-2xl text-sm leading-relaxed shadow-sm ${
                  msg.role === 'bot'
                    ? 'bg-space-700 text-ink border border-space-500/30 rounded-tl-none'
          ...
          {isLoading && (
            <div className="flex justify-start animate-fade-in">
              <div className="bg-space-700 p-4 rounded-2xl rounded-tl-none border border-space-500/30 shadow-sm flex items-center gap-3">
        ...
        {/* Input */}
        <div className="p-4 border-t border-space-500/30 bg-space-800">
