...
import { Users, GraduationCap, Mail, Instagram } from 'lucide-react';
...
const Officers: React.FC = () => {
  ...
  return (
    <div>
      ...
      {/* Featured exec strip */}
      {executive.length > 0 && (
        <section className="py-6 bg-space-950/60 border-y border-space-500/30">
          <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            <div className="section-label inline-flex mb-6">Executive Board</div>
      ...
      <section className="py-6 px-4 sm:px-6 lg:px-8 max-w-7xl mx-auto">
        ...
        {filtered.length === 0 ? (
          <div className="text-center py-7 text-ink-muted">
          ...
          <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-5">
            {filtered.map((officer, i) => (
              <motion.div key={officer.id} initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.4, delay: (i % 6) * 0.08 }}
                ...
                {/* Color accent bar */}
                <div className="h-1 -mx-6 -mt-6 mb-6" style={{ background: officer.category === 'Executive' ? 'linear-gradient(to right, #EE2624, #9C1D20)' : officer.category === 'Advisor' ? 'linear-gradient(to right, #574E8F, #3f3870)' : 'linear-gradient(to right, #005DAA, #004e91)' }} />

                <div className="flex items-start gap-4">
                  <div className="w-16 h-16 rounded-2xl overflow-hidden border-2 border-space-500/60 group-hover:border-electric-300/50 transition-all flex-shrink-0">
                    {officer.imageUrl ? (
                      <LazyImage src={officer.imageUrl} alt={officer.name} className="w-full h-full object-cover" />
                    ) : (
                      <div className="w-full h-full bg-space-700 flex items-center justify-center text-ink-muted text-xs font-bold">
                  ...
                  <div className="flex-1 min-w-0">
                    <h3 className="font-bold text-ink truncate group-hover:text-electric-400 transition-colors">{officer.name}</h3>
                    <div className="text-sm text-ink-muted mt-0.5">{officer.role}</div>
                    <div className="flex items-center gap-2 mt-2 flex-wrap">
                      <span className={`badge ${CATEGORY_COLORS[officer.category] || 'badge-blue'}`}>{officer.category}</span>
                      {officer.grade && (
                ...
                {(officer.email || officer.instagram) && (
                  <div className="flex gap-2 mt-4 pt-4 border-t border-space-500/40">
                    {officer.email && (
                      <a href={`mailto:${officer.email}`} className="p-2 rounded-lg bg-space-700/40 text-ink-muted hover:text-electric-500 hover:bg-electric-100 transition-all">
                        <Mail size={14} />
                      </a>
                    )}
                    {officer.instagram && (
                      <a href={officer.instagram} target="_blank" rel="noopener noreferrer" className="p-2 rounded-lg bg-space-700/40 text-ink-muted hover:text-electric-500 hover:bg-electric-100 transition-all">
                        <Instagram size={14} />
                      </a>
                    )}
                  </div>
