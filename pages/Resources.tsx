...
import { BookOpen, Download, ExternalLink, Search, Lock, Trophy, Code, PenTool, Users, X } from 'lucide-react';
...
const QUICK_LINKS = [
  ...
  { name: 'TSA Competition Rules',   url: 'https://tsaweb.org/competitions-programs/tsa-competitions', desc: 'All official 2024–2025 competition guidelines' },
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-3">
            {QUICK_LINKS.map((link, i) => (
              <motion.a key={i} initial={{ opacity: 0, y: 15 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.3, delay: i * 0.06 }}
                href={link.url} target="_blank" rel="noopener noreferrer"
                className="card flex items-start gap-3 group hover:border-electric-300/40">
                <ExternalLink size={16} className="text-electric-500 flex-shrink-0 mt-0.5" />
                <div>
          ...
          <div className="flex gap-3 flex-wrap">
            <div className="relative">
              ...
              <input type="text" value={search} onChange={e => setSearch(e.target.value)} placeholder="Search resources..."
                className="bg-space-700/50 border border-space-500/60 rounded-xl pl-9 pr-3 py-2 text-sm text-ink placeholder-ink-muted focus:border-electric-500 focus:outline-none focus:ring-1 focus:ring-electric-500/30 transition-all w-52" />
