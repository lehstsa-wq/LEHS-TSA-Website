...
const Login: React.FC = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (!isAuthError) {
        console.error("Non-auth error details:", err);
      }
      ...
      <div className="w-full max-w-md bg-white dark:bg-dark-surface border border-gray-200 dark:border-dark-border rounded-xl p-8 shadow-2xl animate-fade-in transition-colors duration-300">
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          ...
          <button 
            ...
            className="w-full bg-accent-blue hover:bg-accent-purple text-white font-bold py-3 px-6 rounded-lg transition-all duration-300 flex items-center justify-center shadow-lg shadow-accent-blue/20 hover:shadow-accent-purple/40 disabled:opacity-50 disabled:cursor-not-allowed hover:scale-[1.02] hover:-translate-y-1 hover:-rotate-1 group"
          >
            {isSubmitting ? (
              <span className="w-5 h-5 border-2 border-white/30 border-t-white rounded-full animate-spin"></span>
            ) : (
              <>
                <LogIn size={18} className="mr-2 group-hover:rotate-12 transition-transform duration-300" /> Sign In
              </>
            )}
          </button>
        </form>
        ...
        <div className="mt-8 pt-6 border-t border-gray-200 dark:border-dark-border text-center">
          <p className="text-gray-600 dark:text-gray-400 text-sm">
            Need an account?{' '}
            <Link to="/signup" className="text-accent-blue hover:text-accent-purple font-medium transition-colors hover:underline">Join with Code</Link>
          </p>
          <p className="text-xs text-gray-400 mt-4">v1.2 (Recovery Mode Active)</p>
        </div>
      </div>
    </div>
  );
